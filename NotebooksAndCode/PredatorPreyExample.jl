...
#hand-crafted model selector p(m|w)
#
#This will map the animal sizes to models m1, m2, m3 in the following way
#
# w2, w3, w4 -> m1 (small animals)
# w6, w7, w8 -> m2 (medium animals)
# w10, w11, w12 -> m1 (large animals)
#
function pmgw_handcrafted(w_values)
    m = div(w_values-2,4)+1  #compute mapping

    n_m = length(unique(m))
    n_w = length(w_values)

    #initialize with eps() to make sure all values are nonzero
    pmgw = zeros(n_m, n_w) + eps()
    
    #set correct model index in each column of pmgw
    map((w)->pmgw[m[w],w]=1, 1:n_w)
...
#
# m1: uniform dist. over a ∈{sneak up w2, sneak up w3, sneak up w4}
# m2: uniform dist. over a ∈{ambush w6, ambush w7, ambush w8}
# m3: Kronecker delta over a = flee
#
function pagm_handcrafted(a_values, m_values)
    ...
    #m2 - uniform over the specific ambush actions: ambush w6, ambush w7, ambush w8
    pagm[4:6,2] = 1
